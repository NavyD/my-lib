 ...
 *<p>
 * 实现：为每个元素关联一个索引，并使用数组存储索引
 * 再使用一个索引数组的映射数组存储索引与索引数组堆序号的关系
 *</p>
 * 优先队列的实现是将索引数组堆有序化，使用索引通过映射数组找到该索引在堆中的位置
 *<p>
 * 时间复杂度：与logN成正比 ,sink()与swim() 的复杂度是lgN</p>
 * @author Navy D
 * @date 20170814125541
 * @param <Key>
 */
public class IndexMaxPQ<Key extends Comparable<? super Key>> implements Iterable<Integer> {
	// pq堆中元素数量
	private int n;
	// 最大的索引不能超过maxN-1,数组元素最多maxN，索引从0开始
	private int maxN;
	/**
	 * 结构：k为索引，n为索引k堆有序化后的下标 pq[n]=k;qp[k]=n;keys[k]=item
	 */
	// 保存引用索引的堆数组，代替引用作为堆
	// pq存储的是堆中的位置关联的索引值
	private int[] pq;
	...
	public void delete(int k) {
		 ...
		 * keys[pq[index]]存在，改变的是index在sink()后变成对应另外的索引值a,pq[index] = a;
		 * qp[a] = index;只有qp[k]没变，keys[k]没变
		 */
		keys[k] = null;
		qp[k] = -1;
	}
